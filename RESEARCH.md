...
| Proactive outreach | Adopt, gated | Quiet hours 00:00–07:30, at most 5 sends a day (8 in `drill_sergeant`, 2 in `gentle`), ladder 1→4, reset on reply; user-set reminders bypass quiet hours and the cap; `sick` and `travel` days pause the meal, protein and fiber triggers. |
...
`integrations/whoop.py`: server-side code flow with a random, single-use `state` (43 url-safe characters, valid 10 minutes); Fernet-encrypted tokens; a token is refreshed when it is within a minute of expiry or after a 401, one refresh at a time per user, the new pair stored before it is used; `limit=25` pagination; webhook verification by constant-time compare over the raw body with the timestamp within five minutes; the object is fetched and stored before the 200 is returned (no queue); deduplication on `trace_id` (the last 512, in memory) and by upsert on `(user_id, source, external_id)`; `recovery.updated` carries the sleep UUID, so the sleep is fetched, its `cycle_id` read and `/v2/cycle/{id}/recovery` fetched; the 30-minute sync, from a day before the last sync, covers missed deliveries; kJ→kcal and ms→minutes at ingest; `score_state` checked first (`PENDING_SCORE` skipped, `UNSCORABLE` stored without a score; naps stored but not coached); tokens are deleted with everything else by `/forget_me` (a per-provider disconnect that calls `DELETE /v2/user/access` is a follow-up). Why: webhooks are the only way to send the post-workout analysis "within minutes" the brief requires, but WHOOP says deliveries can be missed, so polling stays. The 10-member cap is accepted; approval is filed early.
...
Two ingestion paths, not five. `integrations/withings.py`: OAuth, `getmeas` decoding, the `notify` subscription, and a webhook that treats every POST as a hint: its `startdate`/`enddate` are ignored, the readings are re-fetched with the user's token from the stored cursor (`lastupdate` an hour before the last sync), at most once a minute per user, and the cursor is advanced only by the 30-minute sync, so a spoofed POST (user ids are small integers) can neither skip a weigh-in nor burn the quota; 30 days are imported on connect and the sync covers what a notification missed, because Withings never redelivers. `integrations/apple_health.py`: `POST /webhooks/apple-health/<webhook_token>` accepting the Health Auto Export dialect (detected by `data.metrics`) and a minimal native `samples[]` dialect a hand-built Shortcut can emit; a 32-byte random per-user token; an optional `X-Strikt-Secret` header that, when present, must equal the token (constant-time compare; Shortcuts and Health Auto Export cannot compute a signature); units normalized at ingest; idempotent — measurements on (source, type, instant, metric), sleeps and workouts on their id or a hash of start and end. Every source normalizes into `MeasurementEvent`, `SleepEvent` and `WorkoutEvent` behind one `Integration` protocol, so Oura, Garmin and Google Health are later modules. A scale reading is compared with the 7-day average, never commented on alone. Rejected: Renpho, Eufy and Xiaomi modules (no API), the legacy Fitbit API (dying this month), Garmin (no self-serve access), Google Health at launch (privacy review).
